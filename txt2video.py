...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if accelerator.is_main_process:
        ...
        save_path = f"./data_folder/inference/sample-{global_step}-{str(seed)}-{now}/{skeleton_name}.gif"
        save_videos_grid(samples, save_path)
        logger.info(f"Saved samples to {save_path}")

        # Save the 'save_path' to the file
        with open(f"/content/ict3104-team13-2023/data_folder/inference_result/save_path.txt", "w") as f:
            f.write(save_path)
